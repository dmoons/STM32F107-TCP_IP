...
void OTA_start(unsigned short total_packet)
{
	...
	gOTA.ota_flah_write_addr 	= APP2_CODE_ADDR;
...
unsigned char OTA_Process(unsigned char *ota_buf,unsigned short length)
{
	...
	if(gOTA.ota_flah_write_addr - APP2_CODE_ADDR >= APP2_CODE_SIZE )
		return 3;
...
{
	unsigned char i,version_buffer[APP2_VERSION_SIZE];
	...
	IntGetFourChar(&version_buffer[11],APP2_CODE_ADDR);
	SPI_Flash_Write(APP2_VERSION_ADDR,version_buffer,APP2_VERSION_SIZE);	
